...
// list, an array xor_alloc to keep track which blocks have been
...
void bd_print() {
    for (int k = 0; k < nsizes; k++) {
        LOG("size %d (blksz %ld nblk %d): free list: ", k, BLK_SIZE(k), NBLK(k));
...
void kmfree(void *p) {
    ...
    for (k = size(p); k < MAXSIZE; k++) {
        ...
        bit_invert(bd_sizes[k].xor_alloc, bi / 2);           // free p at size k
...
int bd_initfree_pair(int k, int bi, bool is_buddy) {
    int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
    int free = 0;
...
int bd_mark_data_structures(char *p) {
    ...
    // LOG("bd: %d meta bytes for managing %ld bytes of memory\n", meta,
    //        BLK_SIZE(MAXSIZE));
    bd_mark(bd_base, p);
    return meta;
}
...
void kinit(uint64_t base, uint64_t end) {

    char *p = (char *)ROUNDUP((uint64_t)base, LEAF_SIZE);
    int sz;

    init_spinlock(&lock, "buddy");
    bd_base = (void *)p;
    ...
    // LOG("bd: memory sz is %ld bytes; allocate an size array of length %d\n",
    //        (char *)end - p, nsizes);
    ...
    // initialize free list and allocate the xor_alloc array for each size k
    for (int k = 0; k < nsizes; k++) {
